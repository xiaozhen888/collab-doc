...
/*    @Override
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        System.out.println("原始 Authorization 头: [" + header + "]");

        if (header != null && header.startsWith("Bearer")) {
            String token = header.substring(7);
            System.out.println("===== 调试信息 =====");
            System.out.println("原始 header: [" + header + "]");
            System.out.println("提取的 token: [" + token + "]");
            System.out.println("token 长度: " + token.length());
            System.out.println("token 字符编码: ");
            for (int i = 0; i < token.length(); i++) {
                char c = token.charAt(i);
                if (c == ' ') {
                    System.out.println("位置 " + i + " 是空格，ASCII: " + (int)c);
                }
            }
            System.out.println("=================");
...
//            String token = header.substring(start);
            System.out.println("提取的 token 长度: " + token.length());
            System.out.println("提取的 token 前后: [" + token.substring(0, Math.min(20, token.length())) + "...]");
